...
def add_suggestions(
    conn: sqlite3.Connection, suggestions: Sequence[Suggestion], *, analysis_id: int | None, model: str
) -> list[Suggestion]:
    ...
    for suggestion in suggestions:
        ...
        stored.append(suggestion.with_id(int(cur.lastrowid)))
    conn.commit()
    return stored


def list_suggestions(
    conn: sqlite3.Connection, *, ticker: str | None = None, status: str | None = PENDING, limit: int = 50
) -> list[Suggestion]:
...
def resolve(conn: sqlite3.Connection, suggestion_id: int, status: str, alert_id: int | None = None) -> bool:
    cur = conn.execute(
...
def pending_count(conn: sqlite3.Connection, ticker: str | None = None) -> int:
